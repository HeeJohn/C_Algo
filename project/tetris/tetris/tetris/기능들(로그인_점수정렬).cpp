...
void gotoxy(short x, short y) //x값을 2x로 변경, 좌표값에 바로 문자열을 입력할 수 있도록 printf함수 삽입  
{
    COORD pos = { (2 * x),y };//가로로 한칸 움직인 길이를 세로 한 칸 움직인 길이와 비교를 해본 결과 세로 한 칸의 길이가 두배 길다 
